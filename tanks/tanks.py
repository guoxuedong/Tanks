...
class Bonus():
    ...
    # bonus types
    (BONUS_GRENADE, BONUS_HELMET, BONUS_SHOVEL, BONUS_STAR, BONUS_TANK, BONUS_TIMER,BONUS_FASTER_SPEED, BONUS_SLOWLY_SPEED, BONUS_FASTER_BULLET, BONUS_SLOWLY_BULLET, BONUS_MULTI_BULLET) = range(11)
    ...
    def __init__(self, level):
        ...
        self.bonus = self.BONUS_MULTI_BULLET
        '''
        self.bonus = random.choice([
            self.BONUS_FASTER_SPEED,
            self.BONUS_SLOWLY_SPEED, 
            self.BONUS_FASTER_BULLET, 
            self.BONUS_SLOWLY_BULLET
            self.BONUS_MULTI_BULLET
            ])
        self.bonus = random.choice([
            self.BONUS_GRENADE,
            self.BONUS_HELMET,
        ...
        if (self.bonus < 6):
            ...
            self.image = sprites.subsurface(16*2*(self.bonus%6), 32*7, 16*2, 15*2)
...
class Tank():
    ...
    def fire(self, forced = False):
        ...
        # TODO
        # if eat faster bullet
        if self.fastbullet == 1:
            bullet.speed = 16
        elif self.fastbullet == 2:
            bullet.speed = 2 
        else:
            bullet.speed = 5
...
class Game():
    ...
    def __init__(self):
        ...
        # load custom font
        self.font = pygame.font.Font("fonts/freesansbold.ttf", 16)
    ...
    def triggerBonus(self, bonus, player):
        ...
        if bonus.bonus == bonus.BONUS_GRENADE:
            ...
            self.slowlyBullet(player, True, 10000)
        ...
        elif bonus.bonus == bonus.BONUS_FASTER_SPEED:
            self.fasterSpeed(player, True, 10000)
        elif bonus.bonus == bonus.BONUS_SLOWLY_SPEED:
            self.slowlySpeed(player, True, 10000)
        elif bonus.bonus == bonus.BONUS_FASTER_BULLET:
            self.fasterBullet(player, True, 10000)
        elif bonus.bonus == bonus.BONUS_SLOWLY_BULLET:
            self.slowlyBullet(player, True, 10000)
        elif bonus.bonus == bonus.BONUS_MULTI_BULLET:
            self.multiBullet(player, True, 10000)
        bonuses.remove(bonus)

        labels.append(Label(bonus.rect.topleft, "500", 500))

    def multiBullet(self, player, multi = True, duration = None):
        """ multi/normal bullet
        player: player (not enemy)
        fast: true/false
        duration: in ms. if none, do not return to normal bullet automatically
        """
        #TODO
        if multi:
            print "multi bullet"
            player.max_active_bullets  = 8
        else:
            print "return to normal bullet"
            if player.superpowers <= 1:
                player.max_active_bullets = 1
            else:
                player.max_active_bullets = 2

        if multi and duration != None:
            print "add timer to return to normal bullet"
            gtimer.add(duration, lambda :self.multiBullet(player, False), 1)


    def slowlyBullet(self, player, slow = True, duration = None):
        """ slow/normal bullet
        player: player (not enemy)
        slow: true/false
        duration: in ms. if none, do not return to normal bullet automatically
        ...
        if slow and duration != None:
            print "add timer to return to normal bullet"
            gtimer.add(duration, lambda :self.slowlyBullet(player, False), 1)


    def fasterBullet(self, player, fast = True, duration = None):
        """ fast/normal bullet
        player: player (not enemy)
        fast: true/false
        duration: in ms. if none, do not return to normal bullet automatically
        ...
        if fast and duration != None:
            print "add timer to return to normal bullet"
            gtimer.add(duration, lambda :self.fasterBullet(player, False), 1)



    def fasterSpeed(self, player, fast = True, duration = None):
        """ fast/normal speed
        player: player (not enemy)
        fast: true/false
        duration: in ms. if none, do not lower speed automatically
        """
        faster_speed = 8
        normal_speed = 2
        if fast:
            print "faster Speed"
            player.speed = faster_speed
        else:
            print "return to normal Speed"
            player.speed = normal_speed

        if fast and duration != None:
            print "add timer to return to normal speed"
            gtimer.add(duration, lambda :self.fasterSpeed(player, False), 1)

    def slowlySpeed(self, player, slowly = True, duration = None):
        """ slowly/normal speed
        player: player (not enemy)
        slowly: true/false
        duration: in ms. if none, do not return to normal speed automatically
        """
        slowly_speed = 1
        normal_speed = 2
        if slowly:
            print "slowlyer Speed"
            player.speed = slowly_speed
        else:
            print "return to normal Speed"
            player.speed = normal_speed

        if slowly and duration != None:
            print "add timer to return to normal speed"
            gtimer.add(duration, lambda :self.slowlySpeed(player, False), 1)
    ...
    def shieldPlayer(self, player, shield = True, duration = None):
        ...
        if shield and self.timer_uuid_shield == None:
        ...
        if shield and duration != None:
            ...
            #TODO
            if self.timer_uuid_shield == None:
                self.timer_uuid_shield = gtimer.add(duration, lambda :self.shieldPlayer(player, False), 1)
            else:
                gtimer.inc_interval(self.timer_uuid_shield,duration)
    ...
    def nextLevel(self):
        ...
        levels_enemies = (
            ...
            (2,2,0,0), (14,4,0,2), (14,4,0,2), (2,5,10,3), (8,5,5,2),
